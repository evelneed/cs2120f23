...
-- Extra credit answers here
| (Expr.top_exp b),        i => true
| (Expr.bot_exp b),        i => false
...
| a2b2y => (fun f => Prod.snd a2b2y (Prod.fst a2b2y f))
...
inductive Sandwich (Bread Cheese Spread : Type) : Type
| mk ((B : Bread) × ((c : Cheese) ⊕ (s : Spread)))
...
def wheatJam (Sandwich : Type) 
| mk (wheat jam)

def jam_sandwich : Sandwich := mk (wheat jam)
