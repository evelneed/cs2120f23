...
-- Answer below
def funkom {α β γ : Type} (g: β → γ) (f : α → β) := fun a  => g (f a)
...
def mkop {α β : Type} (a : α) (b : β) : α x β 
 (a, b) 
...
Answer here: 
x is right associative because Nat x Nat x Nat is equivalent to (Nat x Nat) x Nat
...
-- Here:
#eval first (triple 5 "evelyn" true)     -- Should return 5 (it does!)
#eval second (triple 4 "marie" true)    -- Should return "marie" (it does!)
#eval third (triple 3 "needham" true)   -- Should return true (it does!)
...
-/
def newTerm : (Nat × String) × Bool := ((1, "Evelyn"), false)
#check newTerm --neewTerm is of type (Nat × String) × Bool