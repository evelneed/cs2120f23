...
def demorgan1  {α β : Type} : (( no α ) ⊕ (no β)) → (α × β → Empty)  
...
-/
def demorgan3 {α β : Type} : ((no α) × (no β)) → ((α ⊕ β) → Empty) :=
fun (noA, noB) => fun aOrB =>
  match aOrB with
| Sum.inl a => noA a
| Sum.inr b => noB b
...
| 0 => Doll.empty
| (Nat.succ n') => Doll.nest (mk_doll n')
...
def add : Nat → Nat → Nat
| m, 0 => m
| m, (Nat.succ n') => Nat.succ (add m n')  -- hint: recursion
...
#reduce sum_f (fun s => s * s) 4 -- expected 30
-- 1 + 4 + 9 + 16 -- 30, 