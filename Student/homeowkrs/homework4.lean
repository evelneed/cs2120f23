...
-- Here
def its_wet {rain sprinkler wet : Type} : (rainToS : rain ⊕ sprinkler) (rain_to_wet : rain → wet) (sprinkler_to_wet : sprinkler → wet) → wet :=
match rainToS with
| sum.inl r => rain_to_wet r
| sum.inr s => sprinkler_to_wet s
...
-- Here:
def sum_elim {α γ β : Type} : (rainToS : α ⊕ γ) (rain_to_wet : α → β) (sprinkler_to_wet : γ → β) → β :=
match rainToS with
| sum.inl r => rain_to_wet r
| sum.inr s => sprinkler_to_wet s
