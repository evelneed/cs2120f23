...
- two and three are equivalent 
- Are #1 and #3 equivalent?
- no 
- Give English explanations of these function types
- one takes a functon that returns a string and then returns a string from that. two take s astring and returns a function that takes in a string and returns a string. three is the same as two I believe. 
- Give some examples of functions of these types
- Two and three are both examples of concatanation while the description givien in classof a caesar splicer was perfect!
...
careful. 
-/
it is of type string to string to string or maybe just string to string
...
this one? confused on this question 
